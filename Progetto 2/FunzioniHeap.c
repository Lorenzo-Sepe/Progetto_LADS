...
//funzioni per il float
  
/**
 * @brief  create new minHeapFloat
 * 
 * @param v 
 * @param cost 
...
/**
 * @brief Create a Min Heap Float object
 * 
 * @param capacity 
 * @return HeapMinimoFloat 
 */
HeapMinimoFloat createMinHeapFloat(int capacity){
    HeapMinimoFloat minHeap = (HeapMinimoFloat) malloc(sizeof(struct MinHeapFloat));
    minHeap->pos = (int *)malloc(capacity * sizeof(int));
    minHeap->size = 0;
    minHeap->capacity = capacity;
...
void minHeapifyFloat(HeapMinimoFloat minHeap, int idx){
    ...
    if (smallest != idx)
    {
        ...
        // Swap nodes
        swapMinHeapNodeFloat(&minHeap->array[smallest], &minHeap->array[idx]);
  
        minHeapifyFloat(minHeap, smallest);
    }
}
...
NodoHeapFloat extractMinFloat(HeapMinimoFloat minHeap){
    ...
    // Store the root node
    NodoHeapFloat root = minHeap->array[0];
  
    // Replace root node with last node
    NodoHeapFloat lastNode = minHeap->array[minHeap->size - 1];
    minHeap->array[0] = lastNode;
...
void decreaseKeyFloat(HeapMinimoFloat minHeap, int v, float cost){
    ...
    while (i && minHeap->array[i]->cost < minHeap->array[(i - 1) / 2]->cost)
    {
        ...
        minHeap->pos[minHeap->array[i]->v] = (i-1)/2;
        minHeap->pos[minHeap->array[(i-1)/2]->v] = i;
        swapMinHeapNodeFloat(&minHeap->array[i], &minHeap->array[(i - 1) / 2]);
